...
Partial Class Reports_PurchaseFromSale
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            LoadPurchaseFromSaleGridData(InvestorID)
    ...
    ''' <remarks></remarks>
    Private Sub LoadPurchaseFromSaleGridData(ByVal InvestorID As Integer)
        ''InvestorID = 155
        Using conn As New OleDbConnection(util.ConnectString)
            Dim cmd As New OleDbCommand()

            cmd.CommandText = String.Format("SELECT CertificateNumber AS CP, ISNULL(APN,'') AS APNPARCEL, ISNULL(DateofSale,'') AS SaleDate, " & _
                                            "FaceValueOfCP, MonthlyRateOfInterest AS Interest FROM genii_user.TR_CP WHERE InvestorID = {0}; ", InvestorID)


            cmd.Connection = conn

            conn.Open()
            Me.gvPurchaseFromSale.DataSource = cmd.ExecuteReader()
            Me.gvPurchaseFromSale.DataBind()
        End Using
    End Sub
